...
public class CreateShopCommand {
    ...
    private static int createShopCommand(CommandContext<CommandSourceStack> context) {
        try {
            ...
            if (heldItem.isEmpty()) {
                player.sendSystemMessage(Component.literal("§cYou must be holding an item to create a shop!"));
                return 0;
            }
            ...
            if (player.level().getBlockEntity(signPos) instanceof SignBlockEntity signEntity) {
                ...
                if (lines.length > 0 && "[SHOP]".equals(lines[0].getString())) {
                    player.sendSystemMessage(Component.literal("§cThis sign is already a shop!"));
                    return 0;
                }
            }
            ...
            if (shopManager != null) {
                ...
                if (success) {
                    player.sendSystemMessage(Component.literal(String.format(
                            "§aShop created! Item: %s, Buy: $%.2f, Sell: $%.2f, Quantity: %d",
                            heldItem.getDisplayName().getString(), buyPrice, sellPrice, quantity
                    )));
            ...
            context.getSource().sendFailure(Component.literal("§cAn error occurred while creating the shop!"));
            return 0;
        }
    }
}
