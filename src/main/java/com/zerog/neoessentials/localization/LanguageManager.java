...
public class LanguageManager {
    ...
    /**
     * Create a minimal emergency fallback language to prevent critical failures
     * Most messages should be loaded from JSON resources
     */
    private Properties createBasicFallbackLanguage() {
        Properties fallback = new Properties();
        
        // Only absolute essentials for system stability
        fallback.setProperty("general.prefix", "&6[NeoEssentials]&r");
        fallback.setProperty("general.no_permission", "&cNo permission");
        fallback.setProperty("general.error", "&cError occurred");
        fallback.setProperty("general.loading", "&7Loading...");
        
        LOGGER.warn("Using minimal emergency fallback language - JSON resources should be loaded instead!");
        return fallback;
    }
    ...
    /**
     * Generate minimal fallback messages for critical system keys only
     */
    private String generateFallbackMessage(String key) {
        ...
        // Only provide fallbacks for absolutely critical system messages
        if (key.equals("general.prefix")) {
            return "&6[NeoEssentials]&r";
        }
        if (key.equals("general.no_permission")) {
            return "&cNo permission";
        }
        if (key.equals("general.error")) {
            return "&cError";
        }
        if (key.equals("general.loading")) {
            return "&7Loading...";
        }
        
        // For everything else, rely on JSON resources
        return null;
    }
