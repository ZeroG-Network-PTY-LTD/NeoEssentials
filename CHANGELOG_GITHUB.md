...
- Root cause of `<[Owner] [Owner] PlayerName>`-style doubled prefixes: `{neoessentials_displayname}`
  ...
  the build.42 changelog note that called LuckPerms "not a conflict" here — under vanilla
...
- Along the way this also fixed a few real latent bugs it uncovered: a printf `%s` vs. SLF4J
  ...
  disk, and a couple of spots that were logging raw session IDs in plaintext.

---

## [1.0.4+build.43] — 2026-08-13

### 🐛 The build.42 Nickname Fix Never Reached Existing Installs, and LuckPerms Ignored Per-World Contexts for Online Players

- build.42 fixed the *shipped default* `chat-format`/`formatTemplates` strings to reference
...
- Config now value-patches any chat-format/formatTemplates entry that's an **exact** match for
  ...
  fix now actually reaches installs that generated their config before build.42.
...
## [1.0.4+build.12-pre-fix] — 2026-07-22
