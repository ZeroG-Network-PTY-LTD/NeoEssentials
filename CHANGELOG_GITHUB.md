...
### Added
- **2026-09-16** — Build 77 ([`1c4cc11c`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/1c4cc11caf760ddaefb9ca2dfe3e14f4b59f440f)) — **Valued permission nodes, used for FTB Chunks claim-limit perks** — NeoEssentials groups could only store plain granted/denied permission strings; some third-party mods (FTB Chunks' `ftbchunks.max_claimed`/`ftbchunks.max_force_loaded`) expect a *number* instead, queried through NeoForge's typed `PermissionNode<Integer>`/`PermissionNode<String>` API — previously NeoEssentials only ever answered `BOOLEAN` nodes and fell through to the default (OP-only) resolver for anything else. Added a per-group `"meta"` map in `permissions.json` (node → value), resolved through the same closest-group-wins inheritance chain regular permissions already use, and wired into NeoForge's Permission API for `INTEGER`/`STRING` node types. The rank ladder below now grants progressively higher FTB Chunks claim limits per tier as an example — only takes effect if a mod actually queries limits through NeoForge's Permission API rather than its own separate config (e.g. FTB Ranks' own `ranks.json`).
- **2026-09-16** — Build 76 ([`6233db30`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/6233db30f89fabfb28f37af0ab85ce82d9ca2e18)) — **Rank ladder with playtime auto-promotion, and `permissions.json` is now a live config** — added a `guest → member → vip → vip+ → mod → admin → owner` group ladder to the default seed, each tier inheriting the one below with progressively higher `/home` slot limits through vip+; `guest` auto-promotes to `member` after a configurable `/playtime` threshold (default 24h, `permissionsAutoPromote` in `config.json`), checked every 5 minutes for online players — vip/vip+/mod/admin/owner stay manually assigned. `permissions.json` itself stopped being a one-time first-boot seed: every group it defines is now re-applied on every server start and `/permissions reload`/`/neoe reload` — config always wins over in-game group edits for a group listed there, so editing the file and reloading actually takes effect instead of silently doing nothing.
- **2026-09-16** — Build 75 ([`6f9a1e6e`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/6f9a1e6e1a9c7b020308fa9fd223ff49d926538f)) — **Third-party command permission gate** — NeoEssentials could only require its own permission nodes on its own commands; a new `command_permissions.json` now lets an admin map an arbitrary command path — including a specific subcommand — added by another mod (or a vanilla command) to a NeoEssentials permission node. A `RegisterCommandsEvent` listener running after every other mod's registration reflectively wraps each targeted command node's executor with a live permission check (deepest configured path wins when both a command and one of its subcommands are mapped), sending the same denial message NeoEssentials' own commands use. Re-reads the mapping on every invocation, so `/neoe reload` picks up a changed permission node with no restart.
- **2026-09-16** — Build 74 ([`1b1dda8f`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/1b1dda8fdcf2764b976ba3a3674729eba0b78876)) — **`/back` is now a bounded undo-stack** — previously remembered only the single most-recent death or teleport-origin location (picked via timestamp comparison); it now pushes every teleport origin and death location onto one per-player LIFO stack (`teleportation.backSettings.maxBackHistory`, default 10), so chaining `/tpa`, `/spawn`, `/home` lets `/back` walk back through each hop in turn. Purely in-memory — cleared on disconnect and server shutdown, so a stale history never resurfaces on rejoin or restart (previous disk-backed persistence was dropped entirely).
- **2026-09-15** — Build 70 ([`059dc153`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/059dc1534146b3f6a6ff12f4694164e43049064a)) — **Fixed /spawn and /home instability near chunk borders, added a PvP system** — Teleport preloading computed the chunk to force-load via a raw `(int)` cast on the destination coordinates, which truncates toward zero instead of flooring; right at a chunk border on the negative side this silently loaded the wrong (neighbouring) chunk, leaving the real destination unloaded and causing a false "unsafe location" — switched to `Math.floor()`, matching how the safety check itself already computed the block position (issue #72). Also added `/pvp on|off|toggle`: a server-wide PvP switch plus a mutual per-player opt-out, temporary newbie-protection immunity for new players, and safe-zone integration reusing the same YAWP region list teleportation already uses to block teleports — a blocked hit cancels cleanly and messages both players instead of just going through.
