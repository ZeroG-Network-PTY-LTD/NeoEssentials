...
All notable changes to this branch are documented here.  
Format: `[version+build] — date`  
This branch targets **Minecraft 26.1.2 · NeoForge 26.1.2.76+** from build.~460
onward; entries before that predate the port and describe the shared 1.21.1
codebase this branch forked from (compatibility: Minecraft 1.21.1 – 1.21.11 ·
NeoForge 21.1.179+).

---

> Build numbers below are **approximate** for all entries except build.482 — the
> real per-commit build counter isn't tracked in git for historical commits, but
> dates and content are accurate to this branch's actual commit history.
>
> **Known gap:** this branch diverged from `Dev-Builds` around 2026-07-04 and has
> not yet merged several `Dev-Builds` fixes landed on 07-06/07-08: permission
...
Covers the remainder of the migration beyond the 07-04 batch below: moved/renamed
...
### 🔀 26.1 Port: Initial API Migration (645 → 103 compile errors)
...
- Added tablist short-tokens/animations (`{tps}`, `{online}`, etc.) usable
  directly in chat, and `#`-prefixed hex gradient stops.
- ChestShop shop items now preserve full item data components (enchantments,
  custom names, modded NBT) instead of just a bare registry ID when assigned via
  `?` right-click. Added a shift+right-click gesture to (re)assign item data on
  an already-configured shop, and a look-at variant of `/chestshop remove`.
...
### 📚 Documentation
- Corrected several wiki pages to match the actual implementation (this work was
  later superseded on the main `Dev-Builds` branch; docs generally aren't
  maintained separately on this port branch — see that branch for current docs).
...
### ✨ Features
- Added `localization.preserveCustomTranslations` config option — lets admins opt
  a server's custom lang file out of the automatic merge/auto-fix logic entirely.
- Added web dashboard configuration schema/getters in `ConfigManager`, and
