...
- **Rules Command Configuration Improvements**
    - Add clear documentation on how to configure `/rules`.
    - Ensure `rules.json` or `settings.json` is generated by default.
    - Allow editing rules directly from the web dashboard.
    - Provide feedback in console when rules fail to load, with guidance on fixing.
- **Improved Split Config Support**
    - Ensure all modules (chat, kits, anti-spam, etc.) generate their JSON files automatically when split configs are enabled.
    - Add validation checks and clear error messages when a required config file is missing.
    - Provide documentation explaining how split configs are structured and where each module's JSON file should be located.
    - Add a migration tool to convert unified configs into split configs safely.
