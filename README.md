...
> NeoEssentials is a comprehensive, config-driven essentials mod for Minecraft NeoForge 26.1-26.1.2 servers (this is the pinned 26.1.x port branch — see [`26.x`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/tree/26.x) for the rolling 26.x build, or [`1.21.x`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/tree/1.21.x) for the 1.21.1 primary build). It provides 100+ commands, GUI tools, advanced administration, a real-time web dashboard, animated tablist, full localization support, and a PlaceholderAPI system — all managed by modular JSON config files.
