...
> NeoEssentials is a comprehensive, config-driven essentials mod for Minecraft NeoForge 26.2 servers (this is the pinned 26.2.x port branch — see the `26.1.x` branch for the pinned 26.1-26.1.2 build, or `1.21.x` for the 1.21.1 primary build; a future 26.3+ release gets its own new branch, same one-branch-per-line pattern). It provides 100+ commands, GUI tools, advanced administration, a real-time web dashboard, animated tablist, full localization support, and a PlaceholderAPI system — all managed by modular JSON config files.
