...
### Added
- **2026-09-16** — Build 77 ([`1c4cc11c`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/1c4cc11caf760ddaefb9ca2dfe3e14f4b59f440f)) — Valued permission nodes — groups can now hold a `"meta"` map (node -> value) for permission systems expecting a number/string instead of a plain grant, wired into NeoForge's Permission API for `INTEGER`/`STRING` node types. Used to give the rank ladder progressively higher FTB Chunks (`ftbchunks.max_claimed`/`max_force_loaded`) claim limits per tier — only effective if a mod queries limits through NeoForge's Permission API rather than its own separate config.
- **2026-09-16** — Build 76 ([`6233db30`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/6233db30f89fabfb28f37af0ab85ce82d9ca2e18)) — Added a `guest -> member -> vip -> vip+ -> mod -> admin -> owner` permission group ladder with progressively higher `/home` limits, plus playtime-based auto-promotion (guest -> member after 24h, configurable). `permissions.json` is no longer a one-time seed — every group it defines is now re-applied on every reload/restart, so editing the file and reloading actually takes effect.
- **2026-09-16** — Build 75 ([`6f9a1e6e`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/6f9a1e6e1a9c7b020308fa9fd223ff49d926538f)) — Third-party command permission gate — map an arbitrary command path (including a specific subcommand) from another mod, or a vanilla command, to a NeoEssentials permission node via `command_permissions.json`. Deepest configured path wins; denial uses the same message NeoEssentials' own commands show; permission-node changes on already-covered commands apply live via `/neoe reload`, no restart needed.
- **2026-09-16** — Build 74 ([`1b1dda8f`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/1b1dda8fdcf2764b976ba3a3674729eba0b78876)) — `/back` is now a bounded in-memory undo-stack (default 10, `teleportation.backSettings.maxBackHistory`) instead of remembering only the single most recent death/teleport location — chaining `/tpa`, `/spawn`, `/home` lets `/back` walk back through each hop in turn. Purely in-memory, cleared on disconnect/restart (no more disk persistence).
