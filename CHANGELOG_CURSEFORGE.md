...
### Added
- **2026-09-16** — Build 77 ([`893aeb43`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/893aeb43375f06591726457eba84e36b69c4b9ec)) — Valued permission nodes — groups can now hold a `"meta"` map (node -> value) for permission systems expecting a number/string instead of a plain grant, wired into NeoForge's Permission API for `INTEGER`/`STRING` node types. Used to give the rank ladder progressively higher FTB Chunks (`ftbchunks.max_claimed`/`max_force_loaded`) claim limits per tier — only effective if a mod queries limits through NeoForge's Permission API rather than its own separate config.
- **2026-09-16** — Build 76 ([`36191b2a`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/36191b2ad4227943f5a3052ca23111badbe75fc2)) — Added a `guest -> member -> vip -> vip+ -> mod -> admin -> owner` permission group ladder with progressively higher `/home` limits, plus playtime-based auto-promotion (guest -> member after 24h, configurable). `permissions.json` is no longer a one-time seed — every group it defines is now re-applied on every reload/restart, so editing the file and reloading actually takes effect.
- **2026-09-16** — Build 75 ([`d09f6fc8`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/d09f6fc8a920f315b8f4353892a4094ac4e041b8)) — Third-party command permission gate — map an arbitrary command path (including a specific subcommand) from another mod, or a vanilla command, to a NeoEssentials permission node via `command_permissions.json`. Deepest configured path wins; denial uses the same message NeoEssentials' own commands show; permission-node changes on already-covered commands apply live via `/neoe reload`, no restart needed.
- **2026-09-16** — Build 74 ([`a3e107c3`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/a3e107c3ecaf7f1b0d3fe44721b0c9c125f8c522)) — `/back` is now a bounded in-memory undo-stack (default 10, `teleportation.backSettings.maxBackHistory`) instead of remembering only the single most recent death/teleport location — chaining `/tpa`, `/spawn`, `/home` lets `/back` walk back through each hop in turn. Purely in-memory, cleared on disconnect/restart (no more disk persistence).
- **2026-09-15** — Build 70 ([`41952fcf`](https://github.com/ZeroG-Network-PTY-LTD/NeoEssentials/commit/41952fcf0c56f44299da685c05e0f557bb4a616b)) — Fixed /spawn and /home instability near chunk borders (issue #72) — teleport preloading was silently loading the wrong chunk right at a border on the negative side, causing a false "unsafe location". Also added a PvP system — `/pvp on|off|toggle`, server-wide switch plus mutual per-player opt-out, newbie protection, and safe-zone integration.
