        ...
        <div className="flex flex-col gap-5">
          <div className="grid grid-cols-[1fr_320px] gap-5">
            <Card title="Groups" icon={Users} accent="cyan">
                ...
                <div key={g.name} className="px-4 py-3 border-b border-[var(--mc-border)] last:border-0 text-[13px] transition-colors hover:bg-[var(--mc-bg-surface-raised)]">
                  ...
                  {isAdmin && (
                    <div className="flex flex-wrap items-center gap-3 mb-1.5 text-[12px] text-[var(--mc-text-secondary)]">
                      ...
                      <button onClick={() => setEditingInherits((s) => ({ ...s, [g.name]: !s[g.name] }))} className="underline transition-colors hover:text-[var(--mc-cyan-400)]">
                      ...
                      {groups
                        ...
                        .map((other) => (
                          <label key={other.name} className="flex items-center gap-1 text-[12px]">
                            <input type="checkbox" checked={g.inherits.includes(other.name)} onChange={() => toggleInherit(g, other.name)} className="accent-[var(--mc-cyan-500)]" />
                            {other.name}
                          </label>
                        ))}
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-1.5">
                    {(g.permissions ?? []).map((p) => (
                      <span key={p} className="font-data text-[11px] px-1.5 py-0.5 rounded-full bg-[var(--mc-bg-surface-raised)] border border-[var(--mc-border)] flex items-center gap-1">
                        {p}
                        {isAdmin && (
              ...
              {groups.length === 0 && <div className="text-center py-8 text-[13px] text-[var(--mc-text-muted)]">No groups configured.</div>}
            </Card>
          ...
          <Card title="Online users" icon={UserCog} accent="cyan">
            {users.length === 0 && <div className="text-center py-8 text-[13px] text-[var(--mc-text-muted)]">No players online.</div>}
            {users.map((u) => (
              <div key={u.username} className="px-4 py-3 border-b border-[var(--mc-border)] last:border-0 text-[13px] transition-colors hover:bg-[var(--mc-bg-surface-raised)]">
                <div className="flex items-center gap-2 mb-1.5">
                  <img src={`https://mc-heads.net/avatar/${u.uuid}/32`} alt="" className="h-5 w-5 rounded-[4px] shrink-0 [image-rendering:pixelated] border border-[var(--mc-border-strong)]" />
